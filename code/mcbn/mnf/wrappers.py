...
class MNFMC(object):
    def __init__(self, N, input_shape, flows_q=2, flows_r=2, use_z=True,  activation=tf.nn.relu, logging=False,
                 nb_classes=1, learn_p=False, layer_dims=(50, 50), flow_dim_h=50, thres_var=1, prior_var_w=1.,
                 prior_var_b=1.):
    ...
    def build_mnf_lenet(self, x, sample=True):
        ...
        with tf.variable_scope(self.opts):
            if not self.built:
                fcinp_dim = self.input_shape[1]
                layer1 = DenseMNF(self.layer_dims[0], N=self.N, input_dim=fcinp_dim, flows_q=self.flows_q,
                                  flows_r=self.flows_r, use_z=self.use_z, logging=self.logging, learn_p=self.learn_p,
                                  prior_var=self.prior_var_w, prior_var_b=self.prior_var_b, flow_dim_h=self.flow_dim_h,
                                  thres_var=self.thres_var)
                self.layers.append(layer1)
                ...
                layer1 = self.layers[0]
            h1 = self.activation(layer1(x, sample=sample))

            if not self.built:
                fcinp_dim = h1.get_shape()[1].value
                layer2 = DenseMNF(self.layer_dims[0], N=self.N, input_dim=fcinp_dim, flows_q=self.flows_q,
                                  flows_r=self.flows_r, use_z=self.use_z, logging=self.logging, learn_p=self.learn_p,
                                  prior_var=self.prior_var_w, prior_var_b=self.prior_var_b, flow_dim_h=self.flow_dim_h,
                                  thres_var=self.thres_var)
                self.layers.append(layer2)
                ...
                layer2 = self.layers[1]
            h2 = self.activation(layer2(h1, sample=sample))
            ...
            if not self.built:
                ...
                layerout = self.layers[2]

        if not self.built:
            self.built = True
        return layerout(h2, sample=sample)
